      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header d-flex align-items-center">
            <h3 class="h4">Edit Bus Form</h3>
          </div>
          <div class="card-body">
            <form class="form-horizontal" action="{{ route('buses.update', ['bus' => $bus->id]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('put')
              <input type="hidden" name="removed" id="removed_photos" value="[]">
              <div class="row">
                <div class="form-group col-4">
                  <label class="form-control-label">License Plate Number</label>
                    <input type="text" name="license" class="form-control mb-3 @error('license') is-invalid @enderror" value="{{ $bus->license }}">
                    @error('license')
                  ...
                  <label class="form-control-label">Bus Type</label>
                    <select name="bustype" class="form-control mb-3 @error('bustype') is-invalid @enderror">
                      ...
                      @foreach($bustypes as $bustype)
                        <option value="{{$bustype->id}}" @if($bustype->id == $bus->bustype->id){{'selected'}}@endif>
...
@section('javascript')
 ...
 <script src="{{asset('plugins/jquery-latest.min.js')}}"></script>
